...
<style>
  .shell{height:100vh;height:100dvh;display:grid;grid-template-columns:16rem 1fr;overflow:hidden}.sidebar{border-right:1px solid var(--line);background:var(--panel);padding:1.2rem;display:flex;flex-direction:column;gap:1.6rem}.brand,.mobile-brand{display:flex;align-items:center;gap:.65rem}.brand i,.mobile-brand i{width:1rem;height:1rem;background:var(--accent);border-radius:35% 65% 55% 45%;transform:rotate(25deg)}.new{display:flex;align-items:center;justify-content:center;gap:.5rem;border:1px solid var(--line);background:var(--bg);color:var(--ink);border-radius:.75rem;padding:.7rem}.local{flex:1}.local span{font-size:.62rem;color:var(--muted);letter-spacing:.12em}.local p{font-size:.83rem}.privacy{font-size:.68rem;line-height:1.5;color:var(--muted)}main{min-width:0;min-height:0;display:flex;flex-direction:column;position:relative;overflow:hidden}header{height:4.5rem;flex:0 0 4.5rem;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;padding:0 1.2rem;background:color-mix(in srgb,var(--bg) 86%,transparent)}.actions{display:flex;gap:.45rem}.mobile-brand{display:none}.dock{position:absolute;left:0;right:0;bottom:2.6rem;padding:1rem clamp(1rem,4vw,4rem);background:linear-gradient(transparent,var(--bg) 35%)}.error{position:absolute;z-index:3;top:5.2rem;left:50%;transform:translateX(-50%);background:#812d2a;color:white;padding:.65rem 1rem;border-radius:.6rem;font-size:.8rem;max-width:80%}@media(max-width:760px){.shell{grid-template-columns:1fr}.mobile-brand{display:flex}header :global(label){display:none}}
</style>
